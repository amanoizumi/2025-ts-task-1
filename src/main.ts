...
export const plant:{
  ...
  name: string;
  price: number;
} /* TODO: 物件型別 */ = { id: 101, name: "琴葉榕", price: 2500 };
...
export type OnShelfPlant = /* TODO: BasicPlant, StockInfo 組合 */ BasicPlant & StockInfo;
...
/* TODO: (參數型別) => 型別 */
export type CalcTotalFn =  (items: Array<CartItem>, coupon?: Coupon) => number;
...
export type UpdateProduct = Pick<Product, 'id' | 'title'> & Partial<Omit<Product, 'id' | 'title'>>;
...
export type TProductType = 'create' | 'update';

export function submitProduct(type: TProductType, product: CreateProduct | UpdateProduct): string {
  if (type === 'create') {
